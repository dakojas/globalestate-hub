...
import { Send, CalendarClock, Loader2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Textarea } from "@/components/ui/textarea";
import MessageBubble from "@/components/agent/MessageBubble";

const AGENT_NAME = "viewing_scheduler";

const SUGGESTIONS = [
  "Naplánuj obhliadku pre klienta Ján Novák na nehnuteľnosť Ibiza Bay Hurghada",
  "Zobraz moje nadchádzajúce obhliadky",
  "Naplánuj obhliadku na budúci týždeň a nastav pripomienku klientovi",
  "Ktoré realitné agentúry máme v systéme?",
];
      ...
      try {
        ...
        if (existing && existing.length > 0) {
          ...
          const conv = await base44.agents.createConversation({
            ...
            metadata: { name: "Plánovač obhliadok" },
          });
          setConversation(conv);
          setMessages([]);
          unsub = base44.agents.subscribeToConversation(conv.id, (data) => {
  ...
  return (
    <div className="max-w-4xl mx-auto h-[calc(100vh-140px)] flex flex-col">
      <div className="flex items-center gap-3 mb-4 px-1">
        ...
        <div>
          <h2 className="text-lg font-semibold text-[#0a1628]">Plánovač obhliadok</h2>
          <p className="text-xs text-gray-500">Harmonogram obhliadok, pripomienky klientom a koordinácia s agentúrami</p>
        </div>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-4 px-1 py-4">
        {messages.length === 0 && (
          <div className="text-center py-12">
            <CalendarClock className="w-10 h-10 text-[#c9a84c]/40 mx-auto mb-3" />
            <p className="text-gray-500 text-sm mb-4">Naplánuj obhliadku, nastav pripomienku klientovi alebo skontroluj koordináciu s agentúrou.</p>
